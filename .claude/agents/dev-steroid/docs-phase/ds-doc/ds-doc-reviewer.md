...
- テンプレの `generation_hints` に「ファイル分割必須」がある場合、実際に分割されていること（1ファイルに全て書かれていたら `needs_revision`）
- 分割されている場合、エントリポイント（index.md）から全サブファイルへの参照が辿れ、参照切れがない
- 各サブファイルの冒頭に「対象範囲」「関連 ID」が明記されている
