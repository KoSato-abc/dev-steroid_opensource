...
## 更新（Write）
- コマンドから指示されたエントリポイント
  （テンプレの `generation_hints` に「ファイル分割必須」がある場合は、エントリポイントを `index.md` として同ディレクトリ配下に機能/モジュール単位でサブファイルを作成する。エントリポイントから各サブファイルへのリンクを設け、整合を保つ）
## 書き込み禁止
...
- テンプレの `generation_hints` に「ファイル分割必須」がある場合は、必ず分割する（1ファイルに全てを書かない）
- 分割時はエントリポイント（index.md）からサブファイルへリンクし、参照切れを作らない
- 各サブファイルの冒頭に「対象範囲」「関連 ID」を明記し、単独で文脈がわかるようにする
## 生成ルール（doc_type 固有）
