...
- テンプレ：`{template_root}/prd-{doc_type_hyphen}/**`（`doc_type` のアンダースコアをハイフンに変換。例：`basic_design` → `prd-basic-design`）
- 出力先：`{docs_root}/{dir}/{doc_type}.md`（`dir` は config のエントリから取得）
- 既存ドキュメント：出力先と同ディレクトリ（存在する場合）
...
2) runner（subagent）を起動：`.claude/agents/dev-steroid/docs-phase/ds-prd/ds-prd-runner.md`
   - runner への指示：テンプレパス、出力先パス、既存ドキュメントパスを明示的に伝える
3) runner 完了後、`state.product.json` の `docs_phase.{doc_type}.status` を `draft` に更新
4) reviewer（subagent）を起動：`.claude/agents/dev-steroid/docs-phase/ds-prd/ds-prd-reviewer.md`
...
### all モードの中断と再開
- 承認待ちで停止した後、ユーザーが承認したら**同一コマンド `/ds-prd all` を再実行**して残りの doc_type を続行する
- 次のアクション出力には、承認後に実行すべきコマンドとして `/ds-prd all` を必ず含める
- 再実行時は `approved` / `not_applicable` の doc_type を自動スキップするため、途中からの継続となる

## git 操作（git.auto=true の場合）
ユーザー承認（`approved`）後:
1) staging: `.dev-steroid/docs/{dir}/`, `state.product.json`
...
- 次の doc_type がある場合：`/ds-prd {next}`
- 全完了の場合：`/ds-spec-init <Issue名>`
